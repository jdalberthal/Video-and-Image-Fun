...
.DESCRIPTION
    ...
    This version uses FFmpeg for broad video format support.

.EXAMPLE
    PS C:\> .\Show-ImagesVideosReindeerSleighFfmpeg.ps1

    Launches the file selection GUI. After selecting files and clicking "Play", the
    script will launch the festive 3D window.

.NOTES
    Name:           Show-ImagesVideosReindeerSleighFfmpeg.ps1
...
# --- Script Metadata ---
$ExternalButtonName = "Reindeer & Sleigh (FFmpeg)"
...
while ($true) {
    ...
    if (-not $dependenciesMet) {
        ...
        $errorForm = New-Object System.Windows.Forms.Form -Property @{
            Text = "Dependency Error"; Size = '480, 220'; StartPosition = "CenterScreen"
        ...
        $errorLabel = New-Object System.Windows.Forms.Label -Property @{
            Text = $errorMessage; Location = '10, 20'; Font = 'Segoe UI, 10'; AutoSize = $true
            MaximumSize = New-Object System.Drawing.Size(($errorForm.ClientSize.Width - 20), 0)
        }
        $errorForm.Controls.Add($errorLabel)
        $linkLabel = New-Object System.Windows.Forms.LinkLabel -Property @{
            Text = "Click here to download FFmpeg"; AutoSize = $true; Font = 'Segoe UI, 10'
        }
    ...
    function Get-VideoDimensions {
        ...
        try {
            ...
            $startInfo.FileName = $ffprobePath; $startInfo.Arguments = $arguments
    ...
    function Get-NextMediaIndex {
        ...
        try {
            ...
            if ($SyncHash.BadMediaFiles.Count -ge $fileCount) { return -1 }
            $startIndex = ($SyncHash.CurrentIndex + 1) % $fileCount; $currentIndex = $startIndex
            do {
                $filePath = $SyncHash.SelectedFiles[$currentIndex]
                if (-not $SyncHash.BadMediaFiles.Contains($filePath)) { $SyncHash.CurrentIndex = $currentIndex; return $currentIndex }
    ...
    function Handle-MediaFailure {
        ...
        if ($playerState.CurrentFilePath -and -not $SyncHash.BadMediaFiles.Contains($playerState.CurrentFilePath)) { $SyncHash.BadMediaFiles.Add($playerState.CurrentFilePath) | Out-Null }
    ...
    function Start-NextMedia {
        ...
        $playerState.CurrentFilePath = $filePath; $playerState.IsFailed = $false
        $ImageExtensions = ".bmp", ".jpeg", ".jpg", ".png", ".tif", ".tiff", ".gif", ".wmp", ".ico"
        ...
        try {
            if ($playerState.IsImage) {
                ...
                $startInfo = New-Object System.Diagnostics.ProcessStartInfo
                $startInfo.FileName = $ffmpegPath; $startInfo.Arguments = $arguments
                $startInfo.UseShellExecute = $false; $startInfo.RedirectStandardOutput = $true; $startInfo.CreateNoWindow = $true
                $process = New-Object System.Diagnostics.Process; $process.StartInfo = $startInfo
                $process.EnableRaisingEvents = $true; $process.Start() | Out-Null
        ...
        } catch { Handle-MediaFailure -PlayerKey $PlayerKey -Reason $_.Exception.Message }
    }
...
"
    ...
    $renderHandler = [System.EventHandler]{
        # First, process any video frames that are ready.
        Animate-FfmpegFrames -SyncHash $SyncHash

        # Then, update the 3D object positions.
    ...
    $window.Add_Closed({
        ...
        foreach ($key in $SyncHash.PlayerStates.Keys) {
            ...
            if ($ps.FfmpegProcess -and -not $ps.FfmpegProcess.HasExited) {
                try { $ps.FfmpegProcess.Kill() } catch {}
            }
            if ($ps.FrameReader) { $ps.FrameReader.Dispose() }
        }
    })
