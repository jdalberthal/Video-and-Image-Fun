...
<Window
    ...
    <Window.Resources>
        <VisualBrush x:Key="FunnelVisualBrush" Stretch="UniformToFill" Viewbox="0,0,1,1">
            <VisualBrush.Visual>
                <Grid Background="LightBlue" Height="600" Width="800">
                    <MediaElement x:Name="videoPlayer" Stretch="Fill" LoadedBehavior="Manual"/>
                    <TextBlock x:Name="FunnelFaceText" Text="" FontSize="10" Foreground="Black" HorizontalAlignment="Center" VerticalAlignment="Center" TextAlignment="Center" Margin="10" IsHitTestVisible="False" />
                    <TextBlock Name="FunnelErrorTextBlock" Foreground="Red" VerticalAlignment="Center" HorizontalAlignment="Center" Visibility="Collapsed" Width="400" Height="400"/>
                </Grid>
            </VisualBrush.Visual>
        </VisualBrush>
    </Window.Resources>
...
"@
            ...
            $SyncHash.MediaPlayer = $Window.FindName("videoPlayer")
            ...
            $SyncHash.FunnelFaceText = $Window.FindName("FunnelFaceText")
            $SyncHash.FunnelErrorTextBlock = $Window.FindName("FunnelErrorTextBlock")
            $SyncHash.MainGrid = $Window.FindName("MainGrid")

            # --- Create the Vortex Mesh (copied from FFmpeg version) ---
            $panelCount = 8
            ...
            $visualBrush = $Window.Resources["FunnelVisualBrush"]
            ...
            if ($TransparentFacesCheckbox.Checked) {
                $visualBrush = $Window.Resources["FunnelVisualBrush"]
                if ($SyncHash.FunnelGeometryModel -and $visualBrush)
                {
                    ...
                    $SyncHash.FunnelGeometryModel.Material = $emissiveMaterial
                }
            }

            # Initialize a state tracker for the single player
            ...
            $SyncHash.HandleMediaFailure = {
                ...
                $SyncHash.Window.Dispatcher.Invoke([action]{
                    $playerState = $SyncHash.PlayerState
                    if ($playerState.IsFailed) { return }
                    $playerState.IsFailed = $true

                    $fileName = if ($SyncHash.MediaPlayer.Source) { $SyncHash.MediaPlayer.Source.Segments[-1] } else { "an unknown media file" }
                    $errorText = "Error: $($fileName)`n$Reason"

                    $SyncHash.FunnelErrorTextBlock.Text = $errorText
                    $SyncHash.FunnelErrorTextBlock.Visibility = "Visible"
                    $SyncHash.MediaPlayer.Visibility = "Collapsed"
                    $SyncHash.MediaPlayer.Stop()
                    ...
                    $recoveryTick = {
                        ...
                        $SyncHash.PlayerState.IsFailed = $false
                        & $SyncHash.MediaEndedHandler -IsRecovery
                    }
            ...
            $MediaFailedHandler = {
                ...
                & $SyncHash.HandleMediaFailure -Reason $reason
            }
            ...
            $MediaOpenedHandler = {
                ...
                $SyncHash.Window.Dispatcher.Invoke([action]{
                    ...
                    $SyncHash.FunnelErrorTextBlock.Visibility = "Collapsed"
                })

                if ($playerState.IsImage) {
                    $Sender.Pause() 
                    if ($SyncHash.SelectedFiles.Count -gt 1) {
                ...
                elseif (-not $Sender.NaturalDuration.HasTimeSpan) {
                    & $SyncHash.HandleMediaFailure -Reason "No duration found (silent failure)."
                }
                else {
                    if ($playerState.ImageTimer) { $playerState.ImageTimer.Stop() }
                }
            ...
            $MediaEndedHandler = {
                ...
                if (-not $IsRecovery) {
                    ...
                    if (($elapsedMilliseconds -lt 2000) -and (-not $playerState.IsImage)) {
                        & $SyncHash.HandleMediaFailure -Reason "Playback failed or ended instantly."
                        return
                    }
                }
                
                if ($SyncHash.SelectedFiles.Count -le 1) {
                    $SyncHash.MediaPlayer.Position = [TimeSpan]::FromSeconds(0)
                    $SyncHash.MediaPlayer.Play()
                }
                else {
                    ...
                    $SyncHash.MediaPlayer.Source = $NewUri
                    if ($SyncHash.RbSelection -notmatch 'Hidden|Custom') {
                        $SyncHash.FunnelFaceText.Text = $NewUri.Segments[-1]
                    }
                    $SyncHash.MediaPlayer.Play()
                }
            }
            ...
            $SyncHash.MediaPlayer.Add_MediaFailed($MediaFailedHandler)
            $SyncHash.MediaPlayer.Add_MediaOpened($MediaOpenedHandler)
            $SyncHash.MediaPlayer.Add_MediaEnded($MediaEndedHandler)

            $SyncHash.VideoUris = $SyncHash.VideoFiles | ForEach-Object { New-Object System.Uri($_, [System.UriKind]::Absolute) }
            
            # Set initial media
            if ($SyncHash.VideoUris.Count -gt 0) {
                $uri = $SyncHash.VideoUris[0]
                $playerState = $SyncHash.PlayerState
                $extension = [System.IO.Path]::GetExtension($uri.LocalPath).ToLower()
                $playerState.IsImage = ($SyncHash.ImageExtensions -contains $extension)
                $SyncHash.MediaPlayer.Source = $uri
            }
            ...
            switch ($SyncHash.RbSelection)
            {
                "Hidden"
                {
                    $SyncHash.FunnelFaceText.Visibility = [System.Windows.Visibility]::Collapsed
                }
                "Filename"
                {
                    ...
                    $SyncHash.FunnelFaceText.Foreground = New-Object System.Windows.Media.SolidColorBrush($mediaColor)
                    $SyncHash.FunnelFaceText.FontSize = $SyncHash.SelectedFontSize
                    $SyncHash.FunnelFaceText.FontFamily = $SyncHash.SelectedFont
                    ...
                    if($Null -ne $WeightFont) { $SyncHash.FunnelFaceText.FontWeight = $WeightFont }
                    if($Null -ne $StyleFont) { $SyncHash.FunnelFaceText.FontStyle = $StyleFont }
                    
                    try { $SyncHash.FunnelFaceText.Text = $SyncHash.VideoUris[0].Segments[-1] } catch {}
                }
                "Custom"
                {
                    $SyncHash.FunnelFaceText.Text = $TextBox.Text
                    $mediaColor = [System.Windows.Media.Color]::FromArgb($SyncHash.TextColor.A, $SyncHash.TextColor.R, $SyncHash.TextColor.G, $SyncHash.TextColor.B)
                    $SyncHash.FunnelFaceText.Foreground = New-Object System.Windows.Media.SolidColorBrush($mediaColor)
                    $SyncHash.FunnelFaceText.FontSize = $SyncHash.SelectedFontSize
                    $SyncHash.FunnelFaceText.FontFamily = $SyncHash.SelectedFont
            ...
            $Window.Add_Closed({
                    if ($SyncHash.PlayerState) {
                        if ($SyncHash.PlayerState.ImageTimer) { $SyncHash.PlayerState.ImageTimer.Stop() }
                        if ($SyncHash.PlayerState.RecoveryTimer) { $SyncHash.PlayerState.RecoveryTimer.Stop() }
                    }
                    if ($SyncHash.MediaPlayer) {
                        $SyncHash.MediaPlayer.Stop()
                        try {
                            $SyncHash.MediaPlayer.Remove_MediaEnded($SyncHash.MediaEndedHandler)
                            $SyncHash.MediaPlayer.Remove_MediaFailed($MediaFailedHandler)
                            $SyncHash.MediaPlayer.Remove_MediaOpened($MediaOpenedHandler)
                        } catch {}
                        $SyncHash.MediaPlayer.Source = $null
                        $SyncHash.MediaPlayer.Close()
                    }
                    $SyncHash.Clear()
                })
