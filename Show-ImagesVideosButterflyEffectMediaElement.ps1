...
.DESCRIPTION
    ...
    This version uses the built-in Windows MediaElement for video playback. As a result, video format support is limited to the codecs installed on the local system (e.g., MP4, WMV, AVI). For broader format support, use the FFmpeg version of this script. The 3D planes are animated procedurally to bounce off the edges of the screen. Users can interact by clicking on the background to randomize the rotation axis of all planes.
.EXAMPLE
...
.NOTES
    Name:           Show-ImagesVideosButterflyEffectMediaElement.ps1
...
$ExternalButtonName = "Spinning Media Planes - MediaElement"
$ScriptDescription = "Displays multiple, independently moving 3D planes, each showing a looping playlist of images and videos using MediaElement."
    ...
    function Start-ButterflyMovement
    {
        ...
        $startupTimer.Add_Tick({
                ...
                for ($i = 1; $i -le 6; $i++)
                {
                    ...
                    # --- Set a safe, random starting position ---
...
"@
    ...
    if ($UseTransparentEffect)
    {
        ...
        foreach ($viewport in $allViewports)
        {
            ...
            $emissiveMaterial.Brush = [System.Windows.Media.Brushes]::White
