...
.DESCRIPTION
    ...
    This version uses FFmpeg for video decoding, providing support for a wide range of video formats without relying on system-installed codecs. The 3D fish are animated procedurally to bounce off the edges of the screen. Users can interact by clicking on the background to randomize the rotation axis of all planes.
.EXAMPLE
    .\Show-ImagesVideosAquariumFfmpeg.ps1
    Launches the file selection dialog. After selection, it launches the WPF window and begins the animation.
.NOTES
    Name:           Show-ImagesVideosAquariumFfmpeg.ps1
...
$ExternalButtonName = "Aquarium Animation (FFmpeg)"
$ScriptDescription = "Displays multiple, independently swimming 3D fish, each showing a looping playlist of images and videos using FFmpeg."
...
function Start-NextMediaItem
{
    ...
    else
    {
        ...
        try {
            ...
            $imageElement.Source = $playerState.WriteableBmp
            $imageElement.Visibility = 'Visible'

            $loopArg = if ($SyncHash.playlist.Count -le 12) { "-stream_loop -1" } else { "" } # 12 = 6 fish * 2 sides

            $psi = New-Object System.Diagnostics.ProcessStartInfo -Property @{
                FileName = "ffmpeg"; Arguments = "-hide_banner -loglevel error $loopArg -i `"$filePath`" -vf scale=${width}:${height} -f rawvideo -pix_fmt bgr24 -";
            ...
            $tickScriptBlock = {
                $pState = $SyncHash.PlayerState["${PlaneIndex}_${Face}"]
                $totalRead = 0
                ...
                if ($totalRead -eq $bytesPerFrame) {
                    $pState.WriteableBmp.Lock(); $pState.WriteableBmp.WritePixels($rect, $buffer, $stride, 0); $pState.WriteableBmp.Unlock()
                }
            }
            $frameTimer.Add_Tick($tickScriptBlock.GetNewClosure())
            $frameTimer.Start()
        } catch {
            Handle-MediaFailure -SyncHash $SyncHash -PlaneIndex $PlaneIndex -Face $Face -Reason $_.Exception.Message
        }
    }
}

function Handle-MediaFailure {
    param($SyncHash, $PlaneIndex, $Face, $Reason)

    $errorBorder = $SyncHash."Plane${PlaneIndex}${Face}ErrorBorder"
    $errorTextBlock = $SyncHash."Plane${PlaneIndex}${Face}ErrorTextBlock"
    $filePath = $SyncHash.PlayerState["${PlaneIndex}_${Face}"].CurrentPath
    $fileName = if ($filePath) { [System.IO.Path]::GetFileName($filePath) } else { "media" }
    $errorTextBlock.Text = "Media Failed: $fileName`n$Reason"
...
function Start-FishMovement
{
    ...
    $startupTimer.Add_Tick({
            ...
            for ($i = 1; $i -le 6; $i++)
            {
                ...
                $SyncHash.PlayerState["${i}_Front"] = @{ IsImage = $false; ImageTimer = $null; RecoveryTimer = $null; FfmpegProcess = $null; WriteableBmp = $null; CurrentPath = $null }
                $SyncHash.PlayerState["${i}_Back"] = @{ IsImage = $false; ImageTimer = $null; RecoveryTimer = $null; FfmpegProcess = $null; WriteableBmp = $null; CurrentPath = $null }
...
"@
    ...
    $SyncHash.Window.Add_Closed({
            ...
            # Stop all media playback processes
...
$BrowseButton.Add_Click({
        $ImageExtensions = "*.bmp", "*.jpeg", "*.jpg", "*.png", "*.tif", "*.tiff", "*.gif", "*.wmp", "*.ico"
        # Extensive list of video extensions supported by FFmpeg
