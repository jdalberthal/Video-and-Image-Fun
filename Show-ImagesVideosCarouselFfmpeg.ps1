...
# --- Script Metadata ---
$ExternalButtonName = "3D Media Carousel`n(FFmpeg)"
$ScriptDescription = "Displays media on vertical panels in a rotating carousel with an undulating wave motion. Uses FFmpeg for broad video format support."
...
while ($true) {
    ...
    $SelectForm.Text = "3D Carousel (FFmpeg) - Media Selector"
...
"@
    ...
    function Start-NextMediaOnPanel {
        ...
        if ($ImageExtensions -contains $extension) {
            ...
            try {
                ...
                $loopArg = if ($SyncHash.SelectedFiles.Count -le $panelCount) { "-stream_loop -1" } else { "" }
    ...
    for ($i = 0; $i -lt $panelCount; $i++) {
        ...
        $panelContainer.Children.Add($panelViewport)
        $panelContainer.Children.Add($panelViewportBack)
        ...
        $transformGroup.Children.Add( (New-Object System.Windows.Media.Media3D.TranslateTransform3D(0, 0, -$carouselRadius)) )
        ...
        $transformGroup.Children.Add($verticalTranslate)

        $panelContainer.Transform = $transformGroup
        $carouselContainer.Children.Add($panelContainer)
