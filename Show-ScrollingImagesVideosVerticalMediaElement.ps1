<#
.SYNOPSIS
    Creates a continuous vertical-scrolling display of selected images and videos using
    MediaElement.
...
.EXAMPLE
    PS C:\> .\Show-ScrollingImagesVideosVerticalMediaElement.ps1
...
.NOTES
    Name:           Show-ScrollingImagesVideosVerticalMediaElement.ps1
...
$ExternalButtonName = "Scrolling Images/Videos `n Vertical - No Ffmpeg"
$ScriptDescription = "Creates a continuous vertical-scrolling display of selected images and videos. Uses the built-in Windows MediaElement."
...
"@
            ...
            $MediaOpenedHandler = {
                ...
                if ($Sender.NaturalVideoHeight -gt 0)
                {
                    ...
                    if ($MaximizeCheckBox.Checked -eq $true)
                    {
                        if ($videoWidth -ge $videoHeight)
                        {
                            # Landscape or square
                            $parentGrid.Width = $workingAreaWidth
                            $parentGrid.Height = $workingAreaWidth / $aspectRatio
                        }
                        else
                        {
                            # Portrait
                            $parentGrid.Height = $workingAreaHeight
                            $parentGrid.Width = $workingAreaHeight * $aspectRatio
                        }
                    }
                    else
                    {
                        # Not maximized
                        $parentGrid.Width = 384
                        $parentGrid.Height = 384 / $aspectRatio
                    }
                    $Sender.Stretch = "Fill"
                }
                else
