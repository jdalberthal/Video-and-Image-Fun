<#
.SYNOPSIS
    Creates a continuous horizontal-scrolling display of selected images and videos using
    FFmpeg.

.DESCRIPTION
    This script launches a GUI to select image and video files, then renders them in a continuous,
    auto-scrolling horizontal display within a WPF window.
...
.EXAMPLE
    PS C:\> .\Show-ScrollingImagesVideosHorizontalFfmpeg.ps1
...
.NOTES
    Name:           Show-ScrollingImagesVideosHorizontalFfmpeg.ps1
...
$ExternalButtonName = "Scrolling Images/Videos `n Horizontal - Uses Ffmpeg"
$ScriptDescription = "Creates a continuous horizontal-scrolling display of selected images and videos. Uses FFmpeg for video decoding."
...
$HelpLabel.Location = New-Object System.Drawing.Point(700, 10)
        ...
        <RichTextBox x:Name="MyRichTextBox" Grid.Row="0" Margin="5" IsReadOnly="True" VerticalScrollBarVisibility="Auto">
            <FlowDocument>
                ...
                <Paragraph TextAlignment="Left" FontFamily="Consolas">
                    ...
                    <Run Text="Pause/Resume      :  P  : Pause/Resume Scrolling"/><LineBreak/>
...
$browseButton.Add_Click({
        ...
        foreach ($row in $DataGridView.Rows)
        {
            ...
            $row.HeaderCell.Style.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter # This was added in the previous request
            $row.HeaderCell.Value = "Play"
        }
        # This line resizes the column to fit the content, which is the missing piece.
...
"@
            ...
            $MediaEndedHandler = {
                ...
                if (-not $IsRecovery)
                {
                    ...
                    {
                        # This handles cases where ffmpeg starts but fails to produce a valid stream
                ...
                if ($isImage)
                {
                    ...
                    $bitmapImage.add_DownloadCompleted({
                            if ($imageElement.Source.PixelHeight -gt 0)
                            {
                                ...
                                $FinishedElement.Width = $FinishedElement.Height * $aspectRatio
                            }
                        })
            ...
            $SyncHash.StartFfmpegStream = {
                ...
                $sourceIdentifier = "ffmpegExit_$($Grid.Name)"
                ...
                Register-ObjectEvent -InputObject $proc -EventName Exited -Action $exitAction -SourceIdentifier $sourceIdentifier | Out-Null
                ...
                $playerState.FfmpegEventId = $sourceIdentifier
            }
            ...
            foreach ($file in $SyncHash.VideoFiles)
            {
                ...
                $grid.Height = if($MaximizeCheckBox.Checked -eq $true) { $workingAreaHeight } else { 384 }
                $grid.Width = $grid.Height # Start square, will be resized
...
$SelectFolderForm.Add_KeyDown({ param($s, $e) if ($e.KeyCode -eq 'F1') { $ReaderPopup = (New-Object System.Xml.XmlNodeReader $XamlHelpPopup); $PopupWindow = [Windows.Markup.XamlReader]::Load($ReaderPopup); $OkButton = $PopupWindow.FindName("OKButton"); $OkButton.Add_Click({ $PopupWindow.Close() }); $PopupWindow.ShowDialog() | Out-Null } })
